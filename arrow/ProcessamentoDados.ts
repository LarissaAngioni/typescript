...
console.log(minasGerais ? 'Minas está na lista' : 'Minas não está na lista');
 
console.log("\nreduce() - Remover todos os elementos repetidos do array estados\n");
let estadoSemRepeticoes = estadosRepetidos.reduce((acumulador: string[], estado) => {
    if (acumulador.indexOf(estado) === -1)
        acumulador.push(estado);
    return acumulador;
    
}, []);
console.dir(estadoSemRepeticoes);
